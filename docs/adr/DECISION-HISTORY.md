...
| 2026-05-13 | [ADR-0004](ADR-0004-vector-store.md) | RAG Embedding Client | **`AsyncAzureOpenAI`** (Azure endpoint) → **`AsyncOpenAI`** (direct OpenAI API). Follows from OpenAI provider switch: team uses direct `OPENAI_API_KEY`; Azure OpenAI is the production upgrade path. |
